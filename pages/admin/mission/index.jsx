...
const index = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get('mission/update/1');
        setMissionData(res.data);
        setFormData({
          photoUrl: res.data?.photoUrl || '',
          missionLanguages: res.data?.missionLanguages || []
        });
      } catch (err) {
  ...
  const handleLanguageChange = (index, field, value) => {
    const updatedLanguages = [...formData.missionLanguages];
    updatedLanguages[index] = { ...updatedLanguages[index], [field]: value };
    setFormData(prev => ({ ...prev, missionLanguages: updatedLanguages }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put('mission/update/1', formData);
      const res = await api.get('/mission/update/1');
      setMissionData(res.data);
      setEditingId(null);
    } catch (err) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6">
        ...
        {formData.missionLanguages.map((lang, index) => (
